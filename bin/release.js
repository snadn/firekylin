...
var npm = 'node ' + process.env.npm_execpath;
...
Promise.resolve().then(function() {
	...
	if (releaseType != 'local' && releaseType != 'update') {
		...
		return Promise.resolve().then(function() {
			...
			if (releaseType == 'online' || releaseType == 'i') {
				...
				var cmd = 'cd ' + outputPath + ' && ' + npm + ' install -d --registry=https://registry.npm.taobao.org';
	...
	if (releaseType == 'update') {
		...
		return Promise.resolve().then(function() {
			var outpkgconf = Object.assign({}, pkgconf, {bundleDependencies: modules});
	...
	if (releaseType != 'local') {
		if (releaseType != 'update') {
			var shrinkwrap = Promise.all([fs.exists(basePath + 'npm-shrinkwrap.production.json'), fs.exists(basePath + 'yarn.lock')]).then(function(exists, yarn) {
				return !exists && !yarn && exec('cd ' + outputPath + ' && npm prune && npm shrinkwrap').then(function() {
